...
const writeBlog = async (req, res) => {
  try {
    ...
    if (!title || !content) {
      return res.status(400).json("tile and content are required");
    }
    const user_id = user.user_id;
    const result = await db("blogs")
      .insert({ user_id, title, description, content })
      .returning(["post_id", "user_id", "title", "description", "content", "created_at"]);
...
const updateBlog = async (req, res) => {
  try {
    ...
    const blog = await db("blogs").select("user_id").where({ post_id: id }).first();
    if (!blog) {
      return res.status(404).json({ message: "Blog not found" });
    }
    const user_id = user.user_id;
    if (user_id!=blog.user_id){
        ...
        return res.status(403).json("You are not authorized to update this blog.");
    }
    const result = await db("blogs")
...
const deleteBlog = async (req,res) => {
    ...
    if (user_id!=blog.user_id){
        // console.log(user_id);
        // console.log(blog.user_id);
        return res.status(403).json("You are not authorized to delete this blog.");
    }
    await db("blogs")
      .delete()
      .where({post_id:id})
